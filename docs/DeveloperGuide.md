...
<div markdown="span" class="alert alert-info">:information_source: **Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the `InternApply`, which `Application` references. This allows `InternApply` to only require one `Tag` object per unique tag, instead of each `Application` needing their own `Tag` objects.<br>
...
* inherits from both `InternApplyStorage` and `UserPrefStorage`, which means it can be treated as either one (if only the functionality of only one is needed).
